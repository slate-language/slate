...
| 4 | ~~the call and return arms: no counted `Buf[Ins]` in `Frame`, fewer `keepable_on`~~ — **DONE, [`call-return`](results/2026-09-23-call-return.md)**: `Frame.code` is an uncounted `Code` (address + count) and the head reads it unchecked; **geometric mean −10.44%** on dev `7b8c4fb` (−6.76% before round two), `fib` −27.1%, `funcs` −17.9%, `calls` −13.7%, `methods` −19.0%. One `undefined` question per call was measured and backed out (+0.3%, layout); **the compile-time proof landed as [`never-absent`](results/2026-09-23-never-absent.md)**, 80% of calls skip the check, mean −7.8% (mostly register allocation) | ~50% of `fib`; call path 4.60% weighted | 2–3%, and it took 10.4% | `vm.sysl`, `run_frames.sysl`, `execute.sysl` |
...
|---|---|---|
| 2026-09-23 | [2026-09-23 — A CALL WHOSE ARGUMENTS CANNOT BE ABSENT SKIPS THE REFUSAL](results/2026-09-23-never-absent.md) | `call-return`'s and `placed-call`'s owed item: `KeptArgs`, the top bit of `CallFn`/`CallMethod`'s `argc`, set where every argument is a literal, an operator's answer, a new value or a local `declare_name` bound (never a parameter, field, element, chain or call answer); the in-place arms skip `keepable_on` per argument — 46 instructions to 1 for `fib(n - 1)`. Reaches **80%** of the 23 programs' calls (unmarked: `dispatch`, `options`, `csv`, which pass parameters on). **Geometric mean −7.83% / −7.29%** in the two orderings, `fib` −15.7%, `funcs` −18.7%, `methods` −10.3% — **but `branches` −21.6% and `globals` −17.9% make no call**: most of it is `run_frames`' register allocation (~650 fewer `sp` references), which a later change there can give back |
