...
| 4 | ~~the call and return arms: no counted `Buf[Ins]` in `Frame`, fewer `keepable_on`~~ — **DONE, [`call-return`](results/2026-09-23-call-return.md)**: `Frame.code` is an uncounted `Code` (address + count) and the head reads it unchecked; **geometric mean −6.76%**, `fib` −14.2%, `funcs` −13.9%, `calls` −12.4%, `methods` −14.1%. One `undefined` question per call was measured and backed out (+0.3%, layout) | ~50% of `fib`; call path 4.60% weighted | 2–3%, and it took 6.8% | `vm.sysl`, `run_frames.sysl`, `execute.sysl` |
...
|---|---|---|
| 2026-09-23 | [2026-09-23 — A FRAME HOLDS ITS CODE AS AN ADDRESS AND A COUNT — profile 6's item 4](results/2026-09-23-call-return.md) | profile 6's item 4: **geometric mean −6.76%** on an alternating best-of-9 against dev `a59aeee`, eight call-heavy programs −12% to −16% (`fib` −14.2%, `funcs` −13.9%, `methods` −14.1%, `calls` −12.4%, `dispatch` −15.5%), nothing slower than +0.8%. `Frame.code` was a counted `Buf[Ins]` — a retain into every frame a call pushed, a retain and three releases at every `Ret`, and a counted local the head bounds-checked on every instruction. It is a `Code` (`*Ins` + count) the unit keeps alive; the head reads `code.ins[pc]` behind its one bound test, which now panics (every chunk ends in `Ret`, pinned); `run_frames` reads the entered chunk in place; `lay_standing` takes the machine and sets the mask in one subtraction. `run_frames` 9,979 → 7,314 machine instructions. **One `undefined` question per call instead of per argument was measured and backed out** — +0.33% and +0.88%, `fib` −2.7% but the call-free loops +4–6%, code layout in the one hot function. Counts unchanged |
