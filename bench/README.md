...
| 4 | ~~the call and return arms: no counted `Buf[Ins]` in `Frame`, fewer `keepable_on`~~ — **DONE, [`call-return`](results/2026-09-23-call-return.md)**: `Frame.code` is an uncounted `Code` (address + count) and the head reads it unchecked; **geometric mean −10.44%** on dev `7b8c4fb` (−6.76% before round two), `fib` −27.1%, `funcs` −17.9%, `calls` −13.7%, `methods` −19.0%. One `undefined` question per call was measured and backed out (+0.3%, layout); **the compile-time proof landed as [`never-absent`](results/2026-09-23-never-absent.md)**, 80% of calls skip the check, mean −2.2%/−0.9%, `funcs` −9% | ~50% of `fib`; call path 4.60% weighted | 2–3%, and it took 10.4% | `vm.sysl`, `run_frames.sysl`, `execute.sysl` |
