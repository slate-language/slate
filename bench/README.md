...
|---|---|---|---|---|
| 1 | ~~a string literal is ONE cell~~ — **DONE, [`construction`](results/2026-09-23-construction.md)**: `alloc` −27.6%, `calls` −25.3%, `csv` −14.7%, mean −4.37% (with row 8 and the class-call `new`). **Still open: an object literal with literal keys needs none** — a `MakeObject` carrying its keys' interned hashes, ~10% of `calls` | 37% of `calls`, ~35% of `alloc`; 2 of 3 allocations per construction | 2–3% | `run_frames.sysl` `PushStr`/`MakeObject`, `code.sysl` `Unit`, `run_compose.sysl`, `table.sysl`, `obj.sysl` |
