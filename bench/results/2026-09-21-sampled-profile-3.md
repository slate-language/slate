...
|---|---|---|---|---|---|
| 1 | **INLINE THE FOUR SUPERINSTRUCTION ARMS** — `fused_load_slot_int`, `fused_add_store_slot`, `fused_load_slot2`, `fused_less_jump_if_false` are out-of-line functions called from the dispatch, each a `bl` and a frame around a handful of instructions. This is 0.0.122's and 0.0.123's move applied to slate's own code | **7.41%**; 14.0% + 3.7% + 2.5% of `branches`, 11.3% of `closures`, 10.3% + 9.7% + 7.4% of `arith`, 9.2% of `funcs` | **3–5%** | INCREMENTAL, **wholly slate's** — **NEW, and the second profile had the numbers and read them as ordinary arm cost** | `run_frames.sysl` (a fused arm's body moves into its `match` arm) |
