...
| ~~**2**~~ | **DONE 2026-09-22 (`lookup-hit-path`), and the ceiling was about right: `fields` −9.1%/−9.7%, `methods` −12.1%/−10.8%, geometric mean −0.60%/−0.75% over two alternating best-of-9 runs. [The write-up](2026-09-22-lookup-hit-path.md).** The diagnosis here named the key comparison and the real cost was next to it: the entry was read TWICE per answer — once to compare the key, once for the value — and the stored hash, which settles the commonest case on its own, was never asked. `field_here`/`set_here` answer both questions from one read, hash first; an own-field hit is answered inside `read_field_site` without calling `field_from_site`. **No shape token was needed and none was added** — a remembered POSITION still costs no invalidation. ~~**THE FIELD AND METHOD LOOKUP, AFTER the inline caches** — `key_here` is the cache's validity check at 12.3% of `methods` and 11.7% of `fields`; a shape token compared as one integer would replace a key comparison~~ | **7.41%**; 30.7% of `fields`, 28.3% of `methods`, 20.8% of `mapset` | **2–4%** | INCREMENTAL, wholly slate's | `site_cache.sysl`, `code.sysl` |
| ~~**3**~~ | **DONE 2026-09-22 (`match-leaf`), and it took about a quarter of the ceiling: `loops` −9.8%/−8.0%, `dispatch` −3.3%/−5.0%, `nested` −1.5%/−3.1%, geometric mean −0.59%/−0.89% over two runs. [The write-up](2026-09-22-match-walk.md).** The diagnosis was right about WHERE and wrong about what to do: `match_walk` is entered once per pattern *and once per pattern inside it*, and the fix is to cut the ENTRIES rather than the function. `match_leaf` answers a name, a wildcard and each literal in an 80-byte frame against the dispatch's 496, and `match_walk` falls off `dispatch`'s profile entirely. **CUTTING THE DISPATCH UP WAS MEASURED FIRST AND IS +0.69%** — moving `match_array`/`match_object` out so they could not be inlined took the function from 10,808 bytes to 4,600 and made `loops` 6.5% SLOWER, a compound pattern paying a real eight-argument call where it had paid none. What is LEFT of the item is the half the note below names: compiling a simple `for` head into the instruction so the matcher is never entered, which is codegen and a separate item. ~~**`match_walk` — `for` heads, `match` arms, destructuring** — a `for` over an array with a bare-name head does not need the pattern matcher at all~~ | **4.49%** + 1.00%; 47.7% of `loops`, 24.2% of `nested`, 18.4% of `dispatch` | **2–3%**, and it took ~0.75% | INCREMENTAL, wholly slate's | `match.sysl`, `run_frames.sysl`, `run_compose.sysl` |
| ~~**4**~~ | **DONE 2026-09-22 (`module-cells`), and it took about TWICE the ceiling: `globals` −35.8%/−36.1%, geometric mean −1.82%/−1.87% over two alternating best-of-9 runs. [The write-up](2026-09-22-module-cells.md).** The diagnosis named the hashing and the real bill was larger: an `assign_name` is **three** hash lookups (`has`, `get_or`, `put`), so `total = total + i * 2 - 1` was ten a turn, not six. The table `LoadDef` already reads is opened to a file's own top-level `val` and `var` — `LoadCell`/`DeclareCell`/`StoreCell`, an empty cell reading `Undefined` and falling back to the lookup, the module's scope written through on every store — and the ten become one. `Map.find`, both `Map.get`s, `lookup` and `assign_name` all fall off `globals`' sampled profile; the program goes 19x qjs to 12.1x. **`branches` reads +4.2%/+4.9% and holds no module-level variable at all** — three more arms in `run_frames`, which is item 5's subject. ~~**MODULE-LEVEL `var` CELLS, `StoreDef`** — confirmed again: the `Map<string,*>` work is 36.6% of `globals` and the `free`/`malloc` churn another ~13%, so still about half of one program~~ | **3.91%**, and ~50% of `globals` | **0.6–0.9% of the mean**, and it took ~1.85% | INCREMENTAL, wholly slate's | `defs.sysl`, `slots.sysl`, `code.sysl`, `run_frames.sysl`, `run_compose.sysl` |
...
| — | ~~**Per-call allocation in `calls` and `csv`**~~ | **DONE 2026-09-22 in `sh.sysl.gc` 0.2.4, and it took about two thirds of the per-program ceiling: `alloc` −18.8%, `calls` −15.5%, `csv` −11.9%, geometric mean −2.46% on an alternating best-of-9, with `gc$alloc`'s sampled self time on `alloc.sl` 25.9% → 9.8%. [The write-up](2026-09-22-gc-0-2-4.md).** The bitmap and the `trailing_zeros` are exactly what the proposal named; what is left inside `gc.alloc` is the header write, the list link and the `_bzero`, which is a FORMAT question and the package author's. ~~**INVESTIGATED 2026-09-22 (`gc-alloc`) and the 6.2% has a name: `take_larger`'s LINEAR SCAN of the bin array, up to 87 classes per allocation.** `sample`'s leaf offsets read against `otool -tv` put 95 of `alloc`'s, 78 of `calls`' and 49 of `csv`'s `gc$alloc` samples in one six-instruction loop; `rebuild_free`'s coalescing is what keeps slate's exact small class empty, so it is the steady state. **Confirmed a `sh.sysl.gc` item** — the fix is a two-word bitmap of non-empty classes plus `trailing_zeros`, ceiling ~6% of the mean, and [the write-up](2026-09-22-gc-alloc.md) names the exact functions for a sysl-org agent. slate's own half (one `current()` per allocation instead of three) landed there at −0.62%/−0.28%~~ |
