...
| — | ~~**A BUILTIN PROPERTY READ STILL BUFFERS ITS ONE ARGUMENT**~~ | ~~**The item that was hiding under the row above.** `builtin_property` in `index.sysl` mallocs a `Buf[Value]`, pushes the receiver and calls `call_native_buf` — a malloc and a free **per `xs.length`, `s.length` or `m.size`** — which `native-args` took off every other builtin call in 0.0.60 and left here as one of "the callers that genuinely have one". It does not have one: the receiver is already standing on the operand stack~~ — **DONE 2026-09-23, [`property-reads`](2026-09-23-property-reads.md): −2.59% geometric mean**, `arrays` **−27.8%**, `strwalk` −8.7%, `csv` −5.2%, `strindex` −4.7%; `arrays` 5.5x Lua → **4.0x** and the mean 3.5x → **3.4x**. `read_field_of_kind` takes a `standing` flag and `standing_property` in `execute.sysl` hands the native a one-cell window. **The `free` node this row was found under is GONE** — 369 main-thread samples → 269, and not one malloc or free frame anywhere in the branch's sample. **And `SLATE_PROFILE`'s `length` µs did not move**: the per-builtin timer brackets `run_native`, and the `Buf` was outside it |
