...
// **`.length` is NOT a set's or a map's**, JavaScript having none there either: a set is not a
// sequence and nothing in one is at a position, so the sentence that says the kind has no such name
// is the right answer and both hosts have to say it in the same words.
try
    print(anything(Set([1])).length)
catch e
    print(e.message)

try
    print(anything(Map()).length)
catch e
    print(e.message)

// **A property is not a method**, and the sentence sends the reader to the property rather than
// saying a set cannot count itself.
try
    print(anything(Set([1])).size())
catch e
    print(e.message)

// **A property of a builtin kind is read-only**, and the complaint names the property.
try
    anything(Map()).size = 3
catch e
    print(e.message)
